                        ...
                        <div class="w-full  px-2">
                             <div class="w-full h-full flex ">
                                <div class="w-auto h-full space-y-3 mx-auto ">
                                        ...
                                        <p class=" text-center text-sm text-gray-500">Voulez allez ajouter une serie</p>
                                        <form @submit.prevent="addSerie" class=" space-y-6">
    ...
    <!--Bar search-->
      <div class=" h-16 bg-gray-900 shadow-2xl flex fixed px-2" style=" width: 80%;">
           <div class=" w-1/4 h-full flex items-center">
                <p class=" text-2xl font-semibold text-white">Series</p>
           </div>        
    ...
    <!--Tout les Series-->
        ...
        <div v-if="this.loading"  class="w-full h-full fixed flex items-center bg-gray-300 opacity-60">
            ...
            <div  class=" w-full flex-wrap flex overflow-hidden overflow-y-visible" style=" height: 35rem;">
                <div class=" w-36 h-44 ml-6 mt-6 space-y-4 relative" v-for="serie in series" :key="serie.id">
                    <button @click="deleteSerie(serie.id)"  class="bg-red-500 flex items-center text-white h-8 w-8 rounded-full  absolute right-1 top-8">
                        <svg class="w-6 h-6 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                            xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M6 18L18 6M6 6l12 12"></path>
                    ...
                    <router-link :to="{name:'detailserie', params:{serie: serie.id} }" class=" w-36 h-44 ml-6 mt-6 space-y-4">
                        <div class=" w-full h-1/2 flex">
                            <img class=" w-28 h-28 mx-auto" src="images/folder.png">
                        </div>
                        <div class=" w-full h-1/2 flex">
                            <div class="w-28 h-28 mx-auto">
                                <p class=" font-semibold text-center text-sm">{{serie.titre}}</p>
                            </div>
                        </div>
                    </router-link>
                </div>
...
export default {
    ...
    mounted(){
        axios.get('/api/allseries').then(({data})=>{
                this.series = data
                this.loading = false
    ...
    methods:{
        search(){
            if(this.titre.length > 0){
                axios.get('/api/searchserie/'+ this.titre).then(({data})=>{
                        this.series = data
                }).catch(error=> console.log(error)) 
            }else{
                axios.get('/api/searchserie/'+ this.titre).then(({data})=>{
        ...
        addSerie(){
            let serie = new FormData();
            serie.append('titre', this.form.titre);
            serie.append('date_de_sortie', this.form.date_de_sortie);
            serie.append('qualite', this.form.qualite);
            serie.append('description', this.form.description);
            serie.append('age', this.form.age);
            serie.append('pub', this.form.pub);
            serie.append('image', this.form.image);
            serie.append('video', this.form.video);
            axios.post('/api/saveserie',serie).then(({data})=>{
                this.series = data
                this.AddLoading = 'oui'
                console.log(this.data)
            }).catch(error=> console.log(error))
        ...
        deleteSerie(id) {
            axios.delete('/api/deleteserie/' + id).then(({data})=>{
                this.series = data
                console.log(data)
            }).catch(error=> console.log(error))
        },
