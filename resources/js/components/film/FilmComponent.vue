                        ...
                        <div class="w-full  px-2">
                             <div class="w-full h-full flex ">
                                <div class="w-auto h-full space-y-3 mx-auto ">
                                        ...
                                        <p class=" text-center text-sm text-gray-500">Voulez allez ajouter un film</p>
                                        <form @submit.prevent="addFilm" class=" space-y-6">
                                            <div class="w-full h-full mt-4 space-y-3">
                                                ...
                                                <div>
                                                    <input v-model="form.qualite" maxlength="5" class="border-2 bg-transparent appearance-none  focus:outline-none text-gray-600 px-2 min-w-full py-2 " placeholder="qualite">
                                                </div>
                                                <div>
    ...
    <!--Bar search-->
      <div class=" h-16 bg-gray-900 shadow-2xl flex fixed px-2" style=" width: 80%;">
           <div class=" w-1/4 h-full flex items-center">
                <p class=" text-2xl font-semibold text-white">Films</p>
           </div>        
                    ...
                    <router-link :to="{name:'detailfilm', params:{film: film.id} }" class=" w-36 h-44 ml-6 mt-6 space-y-4">
                        ...
                        <div class=" w-full h-1/2 flex">
                            <div class="w-28 h-28 mx-auto">
                                <p class=" font-semibold text-center text-sm">{{film.titre}}</p>
                            </div>
                        </div>
                    </router-link>
                </div>
    ...
    methods:{
        search(){
            if(this.titre.length > 0){
                axios.get('/api/searchfilms/'+ this.titre).then(({data})=>{
                        this.films = data
                }).catch(error=> console.log(error)) 
            }else{
                axios.get('/api/searchfilms/'+ this.titre).then(({data})=>{
        ...
        addFilm(){
            ...
            film.append('titre', this.form.titre);
            film.append('date_de_sortie', this.form.date_de_sortie);
            film.append('duree', this.form.duree);
            film.append('qualite', this.form.qualite);
            film.append('description', this.form.description);
            film.append('age', this.form.age);
            film.append('pub', this.form.pub);
            film.append('image', this.form.image);
            film.append('video', this.form.video);
            axios.post('/api/savefilm',film).then(({data})=>{
                this.films = data
                this.AddLoading = 'oui'
                console.log(this.data)
            }).catch(error=> console.log(error))
        ...
        deleteFilm(id) {
            axios.delete('/api/deletefilm/' + id).then(({data})=>{
                this.films = data
                console.log(data)
            }).catch(error=> console.log(error))
        },
